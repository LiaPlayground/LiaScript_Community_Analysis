...
websites from Markdown files and integrate naturally with Git workflows, but they require setting up a tool-chain for compiling Markdown into HTML, JavaScript and CSS; thus, they are primarily oriented towards computational and technical content.
...
computational notebook simultaneously --- a principle LiaScript generalizes
...
collaborative course development~\cite{Schroeder2020}.
...
available features in a system far exceeds those
actually used.
...
Awad et al.~\cite{Awad2019} observed similarly uneven utilization of LMS
...
A systematic review by Feng et al.~\cite{Feng2025} identifies \emph{Effort
Expectancy} --- the perceived complexity of using a feature --- as the most
frequently reported adoption-relevant construct across higher-education
technology studies, alongside Performance Expectancy as the pivotal driver
of adoption decisions.
This suggests that features requiring significant authoring effort face
an adoption barrier even when their pedagogical value is recognised.

Crucially, all of these studies were conducted on server-based systems that
generate usage logs.
No prior work analyses feature adoption in a decentralized, client-side OER
tool where no telemetry is available, and repository content is the only
observable proxy for usage behavior.
...
platform for OER, and Santos-Hermosa et al.~\cite{SantosHermosa2017} analyze
reuse indicators across OER repositories.
Both studies confirm that repository metadata --- commit history, contributor
counts, file content --- provides meaningful signals about authoring behavior,
